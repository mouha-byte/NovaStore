...
const bool _enableAnimations = false; // Set to true for full animations
...
class _SingleProductLandingState extends State<SingleProductLanding> with SingleTickerProviderStateMixin {
  final _scrollController = ScrollController();
  bool _isScrolled = false;
  bool _hasScrolledDown = false; // Track if user has scrolled to enable lazy loading
  
  // Countdown timer state
  late DateTime _offerEndTime;
  Duration _timeRemaining = const Duration();
  
  // Animation controller for brand carousel
  late AnimationController _brandAnimationController;
  
  // Cache for expensive widgets
  Widget? _cachedHeroSection;
  ...
  void initState() {
    ...
    // Initialize brand carousel animation (start paused, play when visible)
    _brandAnimationController = AnimationController(
      vsync: this,
      duration: const Duration(seconds: 40),
    );
    
    // Start animation after initial build
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        _brandAnimationController.repeat();
      }
    });
  }
  ...
  void _handleScroll() {
    ...
    bool newScrolled = _isScrolled;
    bool newHasScrolledDown = _hasScrolledDown;
    ...
    if (offset > 200 && !_hasScrolledDown) {
      newHasScrolledDown = true;
      shouldUpdate = true;
    }
    ...
    if (shouldUpdate) {
      setState(() {
        ...
        _hasScrolledDown = newHasScrolledDown;
      });
    }
  }
  
  // Helper method to conditionally apply animations
  Widget _withAnimation(Widget child) {
    return _enableAnimations ? child : child;
  }
          ...
          SingleChildScrollView(
            ...
            child: Column(
              children: [
                ...
                RepaintBoundary(child: _cachedHeroSection ??= _buildHeroSection(context)),
                RepaintBoundary(child: _buildProblemSection(context)),
                RepaintBoundary(child: _buildSolutionSection(context)),
                RepaintBoundary(child: _buildFeaturesSection(context)),
                
                // Lazy load below sections after scroll
                if (_hasScrolledDown) ...[
                  RepaintBoundary(child: _buildVideoGallerySection(context)),
                  RepaintBoundary(child: _buildHowItWorksSection(context)),
                  RepaintBoundary(child: _buildSponsoredBrandsBanner(context)),
                  RepaintBoundary(child: _buildExpertEndorsementSection(context)),
                  RepaintBoundary(child: _buildLifestyleGallerySection(context)),
                  RepaintBoundary(child: _buildTestimonialsSection(context)),
                  RepaintBoundary(child: _buildCustomerStoriesSection(context)),
                  RepaintBoundary(child: _buildManufacturingGallerySection(context)),
                  RepaintBoundary(child: _buildTrustBadgesSection(context)),
                  RepaintBoundary(child: _buildComparisonSection(context)),
                  RepaintBoundary(child: _buildBeforeAfterSection(context)),
                  RepaintBoundary(child: _buildLimitedOfferSection(context)),
                  RepaintBoundary(child: _buildPricingSection(context)),
                  RepaintBoundary(child: _buildFAQSection(context)),
                  RepaintBoundary(child: _buildFinalCTASection(context)),
                  RepaintBoundary(child: _cachedFooter ??= _buildFooter(context)),
                ] else ...[
  ...
  Widget _buildAutoScrollBanner() {
    ...
    // Simplified - show static items for better performance
    return ListView.builder(
      scrollDirection: Axis.horizontal,
      physics: const NeverScrollableScrollPhysics(),
      itemCount: 20, // Reduced from 1000
      ...
      .custom(
        duration: 40000.ms, // Slower for less CPU usage
        builder: (context, value, child) {
          return Transform.translate(
            offset: Offset(-value * 1500, 0),
            child: child,
          );
        },
      );
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          // Auto-scrolling brands carousel (Optimized)
          SizedBox(
            height: 140,
            child: AnimatedBuilder(
              animation: _brandAnimationController,
              builder: (context, child) {
                return Transform.translate(
                  offset: Offset(-_brandAnimationController.value * 3500, 0),
                  child: Row(
                    children: List.generate(
                      2, // Reduced from 3 to 2 sets for better performance
                      (setIndex) => Row(
                        children: brands.take(8).map((brand) { // Show only first 8 brands
                          return Container(
                            width: 200,
                            ...
                            decoration: BoxDecoration(
                              ...
                              boxShadow: const [
                                BoxShadow(
                                  color: Color(0x1A6366F1), // Optimized color
                                  blurRadius: 20, // Reduced from 30
                                  offset: Offset(0, 8), // Reduced from 10
                                ),
                              ],
                            ),
                            child: Column(
                              ...
                              children: [
                                Container(
                                  ...
                                  decoration: const BoxDecoration(
                                    color: Color(0xFFF3F4F6), // Solid color instead of gradient
                                    shape: BoxShape.circle,
                                  ),
                                  child: ClipRRect(
